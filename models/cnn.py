...
class CNN(object):
    ...
    def __init__(self, input_width, num_input_channels, num_channels, kernel_sizes, strides,
        ...
        prev = num_input_channels
        W_out = input_width
        for i in range(len(num_channels)):
            curr = num_channels[i]
            self.convolutional_layers.append(Conv1d(prev, curr, kernel_sizes[i], strides[i], conv_weight_init_fn, bias_init_fn))
            prev = curr

            W_out = (W_out - kernel_sizes[i]) // strides[i] + 1

        #linear
        self.flatten = Flatten()
        self.linear_layer = Linear(num_channels[-1] * W_out, num_linear_neurons)
